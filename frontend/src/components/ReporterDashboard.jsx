...
function ReporterDashboard({ userName, userEmail, userWallet }) {
  ...
  const detectWaste = async (file) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/detect-waste', {
        method: 'POST',
        body: formData
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/report-waste', {
        method: 'POST',
        body: formData
      });
        ...
        {detectionResult && (
          <div className="detection-result" style={{
            ...
            <div style={{ marginTop: '0.5rem' }}>
              <strong>♻️ How to dispose correctly:</strong>
